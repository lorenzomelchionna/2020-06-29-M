...
public class FXMLController {
    ...
    void doCreaGrafo(ActionEvent event) {
    	...
    	if(boxAnno.getValue() == null) {
    		txtResult.appendText("Nessun anno selezionato!\n");
    		return;
    	}
    	
    	model.creaGrafo(boxAnno.getValue());
    	
    	boxRegista.getItems().addAll(model.allDirectors());
    ...
    void doRegistiAdiacenti(ActionEvent event) {
    	...
    	if(boxRegista.getValue() == null) {
    		txtResult.appendText("Nessun regista selezionato!\n");
    		return;
    	}
    	
    	if(model.adiacentDirectors(boxRegista.getValue()).size() == 0) {
    	...
    	for(Director d : model.adiacentDirectors(boxRegista.getValue())) {
